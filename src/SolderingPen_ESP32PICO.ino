...
#include <U8g2lib.h>  // https://github.com/olikraus/u8g2   //with custom u8g2_fonts.c
...
uint8_t MenuScreen(const char *Items[][language_types], uint8_t numberOfItems, uint8_t selected) {
  ...
  bool isTipScreen = ((Items[0][language] == "烙铁头:") || (Items[0][language] == "Tip:") || (Items[0][language] == "烙鐵頭:"));
...
uint16_t denoiseAnalog(byte port) {
  ...
  if (adc_sensor.readMiliVolts() < 170) {
    for (uint8_t i = 0; i < 32; i++) {  // get 32 readings 得到32个读数
      ...
      value = constrain(1.5 * raw_adc - 173, 20, 150);
